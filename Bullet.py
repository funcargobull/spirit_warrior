...
class Bullet(pygame.sprite.Sprite):
    def __init__(self, x, y, angle, speed, size, damage, relation, timelife=None):
        ...
        if timelife is not None:
            self.image = pygame.transform.rotate(load_image('picturies/weapons/bullet_knife.png', size), degrees(angle))
        elif relation == 'friend':
            self.image = load_image('picturies/weapons/bullet_39.png', size)
        else:
            self.image = load_image('picturies/weapons/bullet_37.png', size)
